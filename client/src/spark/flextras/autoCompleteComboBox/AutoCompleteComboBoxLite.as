...
package spark.flextras.autoCompleteComboBox
{
     ...
     * <br/><br/> AutoComplete is sometimes called AutoSuggest.  When the user types, a list of options, your dataProvider, is filtered and the typed
     * text is highlighted in the drop down list.  If you want to customize styling in the list, you can create your own itemRenderer to do so.
     * <br/><br/> It is available as a free component, in accordance with the Flextras Licensing Agreement.
     *
    ...
    public class AutoCompleteComboBoxLite extends ComboBox
    {
        ...
        //--------------------------------------------------------------------------
        /**
         * @private This tells us to filter the dataProvider during the next renderer event.
         */
        ...
        /**
         * This property defines the filter function used to filter the <code>dataProvider</code> for AutoComplete purposes. <br/> A filterFunction is
         * expected to have the following signature: <pre>f(item:Object):Boolean</pre> where the return value is <code>true</code> if the specified
         * item should remain in the drop down.
         *
        ...
        /**
         * @private private holder for the type ahead text
         */
        ...
        /**
         * This property keeps track of the characters that the user typed. This value will reset when the user selects an item from the drop down or
         * when the user clears out the text in the AutoComplete input. If your itemRenderer implements the IAutoCompleteRenderer interface, you can
         * access this value from an <code>itemRenderer</code> using the typeAheadText property.
         *
         * @default ''
         *
         * @see spark.flextras.autoCompleteComboBox.renderers.IAutoCompleteRenderer
        ...
        /**
         * @private this is an overriden method that will pass data into the renderer if the renderer implements the IAutoCompleteRenderer interface.
         */
        ...
        /**
         * @private Weird bug w/ our AutoComplete and this functionality Flex 4.5 List tries to scroll the list to a certain position and select that
         * 'saved' position after the dataProvider is refreshed.
         *
         * Of coures, in our AutoComplete, we refresh the dataProvider w/ every keystroke; so in certain situations this causes a bug. Doesn't cause a
         * bug in ComboBox because dataProvider is not refreshed internally.
         *
        ...
        /**
         * @private A helper function for setting the type ahead Text value;
         */
        ...
        /**
         * This is the default filter function for AutoComplete functionality. It uses a regular expression to search through the label text to filter
         * values.
        ...
        /**
         * @private This is a helper function to force the next render event to filter the dataProvider
         */
        ...
        /**
         * @private Very Loosely based off the Halo ACCB filterDataPRovider method. This is a helper function used for filtering the dataProvider. It
         * dispatches events, but for the purposes of this lite component [and time] they are completely undocumented.
         */
        ...
        /**
         * @private this is the change handler for when the user types something into the TextInput Completely rewritten from what happens in the
         * parent.  We don't call super.
         */
        ...
        /**
         * @private handler for when the index changes, which means something was selected (probably) If so, then reset the type ahead text, and filter
         * the dataProvider to reset all items. In the mx AutoComplete we could just store the selectedItem, do the filtering, and reset the
         * selectedItem But, because changing the selectItem/Index is deferred to commitProperties in this implementation that approach doesn't work
         * here.
         *
         * Problem is that this method is triggered from commitProperties and changing the typeAheadText needs to trigger commitProperties again to
         * force the filter of the dataProvider.  But, invalidating properties from inside commitProperties doesn't cause commitProperties to be
         * re-run during the next render event causing me all sort of headaches.
         */
        ...
        private function onTextChange(event:FlexEvent):void
        {
            if (textInput.text == "")
                setTypeAheadText("");
        }
        ...
        /**
         * @private There is a bug if someone hits the escape key  It clears out the text w/o resetting the type ahead text This makes sure that
