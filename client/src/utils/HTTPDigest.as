...
package utils
{
	...
	/**
	 * HTTP Digest relies on the MD5 cryptography class found in the as3corelib package. The as3corelib package can be found here:
	 * https://github.com/mikechambers/as3corelib
	 */
	import com.adobe.crypto.MD5;
	
	/**
	 * This class is used to manually handle HTTPDigest authentication. WARNING: this class will overwrite the request headers of the URLRequest that is
	 * passed into it. This is necessary for proper authentication.
	 */
	...
	public class HTTPDigest extends EventDispatcher
	{
		...
		/**
		 * Initiate the request process. Pass in a callback function to receive the data upon completion.
		 */
		...
		/**
		 * We begin with an unauthorized URLRequest. The correct response should be 401 - unauthorized accompanied by nonce values in the HTTP Header
		 */
		private function requestAuthHeaders():void
		{
			...
			{
				e.target.removeEventListener(HTTPStatusEvent.HTTP_STATUS, statusHandler);
				...
				else
				{
					var error:IOErrorEvent = new IOErrorEvent(IOErrorEvent.IO_ERROR,
															  false,
															  false,
															  "HTTPDigest - Server responded to authorization request with a status of " + e.status + "; expected 200, 201 or 401.",
															  0);
					scope.data = "";
		...
		private function makeAuthorizedRequest(responseHeaders:Array):void
		{
			...
			this.urlRequest.requestHeaders = [new URLRequestHeader("Content-Type", this.urlRequest.contentType), new URLRequestHeader("Authorization",
																																	  "Digest " + "username=\"" + this.user /* Username we are using to gain access. */ + "\", " + "realm=\"" + responseObject.realm /* Same value we got from the server.    */ + "\", " + "nonce=\"" + responseObject.nonce /* Same value we got from the server.    */ + "\", " + "uri=\"" + uri /* URI that we are attempting to access. */ + "\", " + // TODO URI?! vs URL?!
